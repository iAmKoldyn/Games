...
/*Функция sun_enqueue принимает позицию блока в игровом мире,уровень входящего света + добавляет блок в очередь на распространение солнечного света.
В основе лежат пять аргументов: x, y, z, (используют координаты блока) base - начальный индекс очереди, incoming_light- беззнаковый символ,
 представляющий уровень света, который будет добавлен к блоку.
*/
 void sun_enqueue(int x, int y, int z, int base, unsigned char incoming_light)

{
    if (incoming_light == 0)
        return;  // If the incoming light level is zero, return immediately without doing anything.

    if (T_(x, y, z) == WATR)
        incoming_light--; // Decrease incoming light level by one for water blocks.

    if (T_(x, y, z) == RLEF || T_(x, y, z) == YLEF)
    {
        incoming_light--; // Decrease incoming light level by one for leaf blocks.
        if (incoming_light) incoming_light--; // Decrease incoming light level by one more if incoming light is not zero.
    }

    if (SUN_(x, y, z) >= incoming_light)
        return; // If the current sunlight level is already brighter or equal to the incoming light level, return without doing anything.

    if (T_(x, y, z) < OPEN)
        return; // If the block is not transparent, return without doing anything.

    set_sunlight(x, y, z, incoming_light); // Set the sunlight level of the block at the given coordinates to the incoming light level.

    if (sq_next_len >= SUNQLEN)
    {
        sunq_outta_room++;
        return; // If there is no more room in the sunlight queue, increment sunq_outta_room and return without doing anything else.
    }

    for (size_t i = base; i < sq_curr_len; i++)
        if (sunq_curr[i].x == x && sunq_curr[i].y == y && sunq_curr[i].z == z)
            return; // If the block is already queued in the current queue, return without doing anything else.

    for (size_t i = 0; i < sq_next_len; i++)
        if (sunq_next[i].x == x && sunq_next[i].y == y && sunq_next[i].z == z)
            return; // If the block is already queued in the next queue, return without doing anything else.

    sunq_next[sq_next_len].x = x; // Add the block coordinates to the next queue.
    sunq_next[sq_next_len].y = y;
    sunq_next[sq_next_len].z = z;
    sq_next_len++; // Increment the length of the next queue.
}
